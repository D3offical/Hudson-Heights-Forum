...
document.querySelectorAll('[data-modal]').forEach(btn=>btn.onclick=()=>{
 ...
 <p>This frontend is ready to connect to Supabase later.</p>
...
<div class="notice">Connect Supabase to enable real accounts, threads, replies, faction applications, staff roles, moderation, and persistent data.</div>
...
if(form)form.onsubmit=e=>{e.preventDefault();openModal(`<h2>Application Ready</h2>
<div class="notice">The form is complete visually. Connect Supabase to save submissions and let Faction Management review them.</div>
<button class="btn btn-primary full" onclick="document.getElementById('modalBackdrop').classList.remove('show')">CLOSE</button>`)};

const reply=document.getElementById('replyBtn');
if(reply)reply.onclick=()=>demoBackend();
...
const factionData={
 pnv:{name:'PNV / 6Block',borough:'Brooklyn',tier:'Tier 2',status:'Official',members:'20+',standing:'Good Standing',turf:'6Block / Brooklyn',summary:'An official Hudson Heights faction focused on consistent neighborhood roleplay, events, media, and faction-driven story development.'},
 queensbridge:{name:'Queensbridge',borough:'Queens',tier:'Tier 1',status:'Official',members:'18',standing:'Good Standing',turf:'Queensbridge',summary:'A recognized Queens faction built around neighborhood roleplay, community presence, and long-form faction storylines.'},
 hilltop:{name:'HillTop',borough:'Manhattan',tier:'Pending',status:'Under Review',members:'14',standing:'Pending Review',turf:'Harlem',summary:'A faction application currently being reviewed by Faction Management.'},
 mtm:{name:'MTM / StainGang',borough:'Queens',tier:'Tier 3',status:'Official',members:'22',standing:'Good Standing',turf:'Ravenswood / Queens',summary:'An official faction centered around active roleplay, neighborhood identity, regular interactions, and faction events.'}
};
function loadFactionProfile(){
 if(!document.getElementById('factionProfile'))return;
 const key=new URLSearchParams(location.search).get('f')||'pnv';
 const d=factionData[key]||factionData.pnv;
 ['Name','Location','Tier','Status','Members','Standing','Turf','Summary'].forEach(k=>{
   const el=document.getElementById('fp'+k);
   if(el){
     const map={Name:d.name,Location:d.borough,Tier:d.tier,Status:d.status,Members:d.members,Standing:d.standing,Turf:d.turf,Summary:d.summary};
